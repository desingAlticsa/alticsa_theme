<div class="front_page_section front_page_section_features<?php
	$chinchilla_scheme = chinchilla_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $chinchilla_scheme ) && ! chinchilla_is_inherit( $chinchilla_scheme ) ) {
		echo ' scheme_' . esc_attr( $chinchilla_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( chinchilla_get_theme_option( 'front_page_features_paddings' ) );
	if ( chinchilla_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$chinchilla_css      = '';
		$chinchilla_bg_image = chinchilla_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$chinchilla_anchor_icon = chinchilla_get_theme_option( 'front_page_features_anchor_icon' );
	$chinchilla_anchor_text = chinchilla_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( chinchilla_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$chinchilla_css      = '';
			$chinchilla_bg_mask  = chinchilla_get_theme_option( 'front_page_features_bg_mask' );
			$chinchilla_bg_color_type = chinchilla_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $chinchilla_bg_color_type ) {
				$chinchilla_bg_color = chinchilla_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$chinchilla_caption = chinchilla_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $chinchilla_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $chinchilla_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $chinchilla_caption, 'chinchilla_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$chinchilla_description = chinchilla_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $chinchilla_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $chinchilla_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $chinchilla_description ), 'chinchilla_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! chinchilla_exists_trx_addons() ) {
						...
						chinchilla_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
