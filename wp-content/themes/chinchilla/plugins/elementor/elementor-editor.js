...
jQuery( document ).ready(
	function() {
		...
		chinchilla_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="chinchilla_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.chinchilla_post_editor' );
						if ( link.length > 0 ) {
							chinchilla_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.chinchilla_post_editor', function(e) {
						chinchilla_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('chinchilla_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function chinchilla_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
