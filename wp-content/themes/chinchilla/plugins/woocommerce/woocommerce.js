...
jQuery( document ).on( 'action.ready_chinchilla', function() {
	...
	$document.on( 'action.scroll_chinchilla', function() {
		_window_scroll_top = $window.scrollTop();
	} );

	$document.on( 'action.resize_chinchilla', function() {
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.chinchilla_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		chinchilla_set_cookie( 'chinchilla_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	chinchilla_woocommerce_add_inc_dec();
		...
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					chinchilla_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function chinchilla_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_chinchilla', function(e) {
		if (CHINCHILLA_STORAGE['load_more_link_busy']) {
			return;
		}
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		chinchilla_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					chinchilla_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					chinchilla_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		chinchilla_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			chinchilla_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function chinchilla_woocommerce_update_cart(status){
		setTimeout(
			function() {
				...
				if ( w >= CHINCHILLA_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
