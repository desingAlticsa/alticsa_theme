...
jQuery( document ).on( 'action.ready_chinchilla', function() {
		...
		if ( $product.length ) {
			...
			if ( $gallery.length ) {
				$document.on( 'action.resize_chinchilla', function() {
					chinchilla_woocommerce_stretch_fullwidth();
				} );
				chinchilla_woocommerce_stretch_fullwidth();
	...
	function chinchilla_woocommerce_stretch_fullwidth() {
	...
	if ( $body.hasClass('single-product') ) {
		...
		if ( product_wrap.length ) {
			...
			chinchilla_add_filter('chinchilla_filter_masonry_wrap', function( selector ) {
				selector += ( selector ? ',' : '' ) + '.woocommerce-product-gallery__wrapper';
				return selector;
			});
			// Add gallery image to the masonry item selector
			chinchilla_add_filter('chinchilla_filter_masonry_item', function( selector ) {
	...
	if ( CHINCHILLA_STORAGE['add_to_cart_sticky'] ) {

		// Show sticky bar on form is out of the window viewport
		var $form = jQuery( '.summary form.cart' );
		if ( $form.length ) {
			chinchilla_intersection_observer_add( $form, function( item, enter, data ) {
				if ( ! enter ) {
		...
		if ( $footer.length ) {
			chinchilla_intersection_observer_add( $footer, function( item, enter, data ) {
				if ( ! enter ) {
					if ( ! $body.hasClass( 'single_product_bottom_bar_sticky_on' ) ) {
						$body.addClass( 'single_product_bottom_bar_sticky_on' );
					}
		...
		jQuery( '.single_product_bottom_bar_button_select_options' ).on( 'click', function(e) {
			chinchilla_document_animate_to( '.variations_form' );
			e.preventDefault();
			return false;
		} );
	}
