...
if ( ! function_exists( 'chinchilla_array_get_first' ) ) {
	function chinchilla_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'chinchilla_array_get_keys_by_value' ) ) {
	function chinchilla_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'chinchilla_array_delete_by_value' ) ) {
	function chinchilla_array_delete_by_value( $arr, $value ) {
		do {
...
if ( ! function_exists( 'chinchilla_array_from_list' ) ) {
	function chinchilla_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'chinchilla_array_slice' ) ) {
	function chinchilla_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'chinchilla_array_merge' ) ) {
	function chinchilla_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'chinchilla_array_insert_after' ) ) {
	function chinchilla_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = chinchilla_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = chinchilla_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if ( ! function_exists( 'chinchilla_array_insert_before' ) ) {
	function chinchilla_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'chinchilla_array_get_by_key' ) ) {
	function chinchilla_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'chinchilla_generate_id' ) ) {
	function chinchilla_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

// Return first tag from text
if ( ! function_exists( 'chinchilla_get_tag' ) ) {
	function chinchilla_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'chinchilla_get_tag_attrib' ) ) {
	function chinchilla_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'chinchilla_get_css_position_from_values' ) ) {
	function chinchilla_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( chinchilla_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'chinchilla_prepare_css_value' ) ) {
	function chinchilla_prepare_css_value( $val ) {
		if ( '' != $val ) {
...
if ( ! function_exists( 'chinchilla_parse_icons_classes' ) ) {
	function chinchilla_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = chinchilla_fga( $css );
...
if ( ! function_exists( 'chinchilla_get_column_class' ) ) {
	function chinchilla_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'chinchilla_stripslashes' ) ) {
	function chinchilla_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
		...
		if ( is_array( $val ) ) {
			foreach ( $val as $k => $v ) {
				$val[ $k ] = chinchilla_stripslashes( $v );
			}
		} else {
...
// Get GET, POST value
if ( ! function_exists( 'chinchilla_get_value_gp' ) ) {
	function chinchilla_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'chinchilla_get_value_gpc' ) ) {
	function chinchilla_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'chinchilla_get_value_gps' ) ) {
	function chinchilla_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'chinchilla_get_session_value' ) ) {
	function chinchilla_get_session_value( $name, $defa = '' ) {
		global $wp_session;
...
if ( ! function_exists( 'chinchilla_set_session_value' ) ) {
	function chinchilla_set_session_value( $name, $value ) {
		global $wp_session;
...
if ( ! function_exists( 'chinchilla_del_session_value' ) ) {
	function chinchilla_del_session_value( $name ) {
		global $wp_session;
...
// Return a cookie
if ( ! function_exists( 'chinchilla_get_cookie' ) ) {
	function chinchilla_get_cookie( $name, $defa = '' ) {
		return chinchilla_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'chinchilla_set_cookie' ) ) {
	function chinchilla_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'chinchilla_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//chinchilla_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//chinchilla_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'chinchilla' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'chinchilla_hex2rgb' ) ) {
	function chinchilla_hex2rgb( $hex ) {
...
if ( ! function_exists( 'chinchilla_hex2rgba' ) ) {
	function chinchilla_hex2rgba( $hex, $alpha ) {
		$rgb = chinchilla_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'chinchilla_hex2hsb' ) ) {
	function chinchilla_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = chinchilla_rgb2hsb( chinchilla_hex2rgb( $hex ) );
...
if ( ! function_exists( 'chinchilla_rgb2hsb' ) ) {
	function chinchilla_rgb2hsb( $rgb ) {
		$hsb      = array();
...
if ( ! function_exists( 'chinchilla_hsb2rgb' ) ) {
	function chinchilla_hsb2rgb( $hsb ) {
		$rgb = array();
		$h   = round( $hsb['h'] );
...
if ( ! function_exists( 'chinchilla_rgb2hex' ) ) {
	function chinchilla_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'chinchilla_hsb2hex' ) ) {
	function chinchilla_hsb2hex( $hsb ) {
		return chinchilla_rgb2hex( chinchilla_hsb2rgb( $hsb ) );
	}
}
...
if (!function_exists('chinchilla_date_to_sql')) {
	function chinchilla_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
// Display price
if (!function_exists('chinchilla_format_price')) {
	function chinchilla_format_price($price) {
...
if (!function_exists('chinchilla_num2size')) {
	function chinchilla_num2size($num) {
...
if (!function_exists('chinchilla_size2num')) {
	function chinchilla_size2num($size) {
...
if (!function_exists('chinchilla_parse_num')) {
	function chinchilla_parse_num($str) {
...
if ( ! function_exists( 'chinchilla_prepare_macros' ) ) {
	function chinchilla_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'chinchilla_remove_macros' ) ) {
	function chinchilla_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'chinchilla_is_on' ) ) {
	function chinchilla_is_on( $prm ) {
		return is_array( $prm )
					? count( $prm ) > 0
...
if ( ! function_exists( 'chinchilla_is_off' ) ) {
	function chinchilla_is_off( $prm ) {
		return is_array( $prm )
					? count( $prm ) == 0
					: empty( $prm )
...
if ( ! function_exists( 'chinchilla_is_inherit' ) ) {
	function chinchilla_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

// Return truncated string (by chars number)
if ( ! function_exists( 'chinchilla_strshort' ) ) {
	function chinchilla_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = chinchilla_strip_tags( $str );
...
if ( ! function_exists( 'chinchilla_strip_tags' ) ) {
	function chinchilla_strip_tags( $str ) {
...
if ( ! function_exists( 'chinchilla_excerpt' ) ) {
	function chinchilla_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return chinchilla_strwords( chinchilla_strip_tags( $str ), $maxlength, $add );
	}
}


// Return truncated string (by words number)
if ( ! function_exists( 'chinchilla_strwords' ) ) {
	function chinchilla_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
// Unserialize string
if ( ! function_exists( 'chinchilla_unserialize' ) ) {
	function chinchilla_unserialize( $str ) {
...
if ( ! function_exists( 'chinchilla_str_replace' ) ) {
	function chinchilla_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = chinchilla_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( chinchilla_str_replace( $from, $to, chinchilla_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'chinchilla_str_replace_once' ) ) {
	function chinchilla_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'chinchilla_tags_count' ) ) {
	function chinchilla_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'chinchilla_getimagesize' ) ) {
	function chinchilla_getimagesize( $url ) {
		// Remove scheme from url
		$url = chinchilla_remove_protocol_from_url( $url );
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => chinchilla_remove_protocol_from_url( $upload_info['baseurl'] ),
			),
			'child'   => array(
				'dir' => CHINCHILLA_CHILD_DIR,
				'url' => chinchilla_remove_protocol_from_url( CHINCHILLA_CHILD_URL ),
			),
			'theme'   => array(
				'dir' => CHINCHILLA_THEME_DIR,
				'url' => chinchilla_remove_protocol_from_url( CHINCHILLA_THEME_URL ),
			),
		);
...
if ( ! function_exists( 'chinchilla_clear_thumb_size' ) ) {
	function chinchilla_clear_thumb_size( $url, $remove_protocol = true ) {
		$pi            = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = chinchilla_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'chinchilla_add_thumb_size' ) ) {
	function chinchilla_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = chinchilla_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = chinchilla_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = chinchilla_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = chinchilla_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'chinchilla_get_thumb_size' ) ) {
	function chinchilla_get_thumb_size( $ts ) {
		$retina = chinchilla_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'chinchilla_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'chinchilla_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'chinchilla-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'chinchilla_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'chinchilla-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'chinchilla_detect_thumb_size' ) ) {
	function chinchilla_detect_thumb_size( $image ) {
		$ts = '';
		if ( preg_match( '/\-([0-9]{1,4}x[0-9]{1,4})\./', $image, $matches ) && ! empty( $matches[1] ) ) {
			$dim = array_map( 'intval', explode( 'x', $matches[1] ) );
			$ts = chinchilla_get_closest_thumb_size( '', array( 'width' => $dim[0], 'height' => $dim[1] ) );
		}
		return $ts;
	}
}
...
if ( ! function_exists( 'chinchilla_get_closest_thumb_size' ) ) {
	function chinchilla_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'chinchilla_get_attachment_url' ) ) {
	function chinchilla_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = chinchilla_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}
...
if ( ! function_exists( 'chinchilla_get_post_image' ) ) {
	function chinchilla_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'chinchilla_get_post_audio' ) ) {
	function chinchilla_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = chinchilla_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = chinchilla_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = chinchilla_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( chinchilla_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = chinchilla_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'chinchilla_get_post_audio_list' ) ) {
	function chinchilla_get_post_audio_list( $args=array() ) {
...
if ( ! function_exists( 'chinchilla_get_post_audio_list_first' ) ) {
	function chinchilla_get_post_audio_list_first( $src = false, $args = array() ) {
...
if ( ! function_exists( 'chinchilla_get_post_video' ) ) {
	function chinchilla_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = chinchilla_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = chinchilla_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = chinchilla_get_tag( $post_text, '<video', '</video>' );
				if ( empty( $img ) ) {
					$sc = chinchilla_get_tag( $post_text, '[video', '[/video]' );
					if ( empty( $sc ) ) {
						$sc = chinchilla_get_tag( $post_text, '[trx_widget_video', '' );
					}
					if ( ! empty( $sc ) ) {
						$img = do_shortcode( $sc );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp\\:trx-addons\\/video.+"link"\\:"([^"]*)"/i', $post_text, $matches ) ) {
						$img = chinchilla_get_embed_video( $matches[1][0] );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:core-embed\\/(youtube|vimeo|dailymotion|facebook).+"url":"([^"]*)"/i', $post_text, $matches ) ) {
						$img = chinchilla_get_embed_video( $matches[2][0] );	// , true
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:embed {"url":"([^"]*(youtube|vimeo|dailymotion|facebook)[^"]*)"/i', $post_text, $matches ) ) {
						$img = chinchilla_get_embed_video( $matches[1][0] );	// , true
					}
		...
		return apply_filters( 'chinchilla_filter_get_post_video', $img );
	}
}


// Return list videos from post attributes
if ( ! function_exists( 'chinchilla_get_post_video_list' ) ) {
	function chinchilla_get_post_video_list( $args=array() ) {
...
if ( ! function_exists( 'chinchilla_get_post_video_list_first' ) ) {
	function chinchilla_get_post_video_list_first( $src = false, $args = array() ) {
...
if ( ! function_exists( 'chinchilla_get_post_iframe' ) ) {
	function chinchilla_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = chinchilla_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = chinchilla_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'chinchilla_filter_get_post_iframe', $img );
	}
}


// Add 'autoplay' feature in the video
if ( ! function_exists( 'chinchilla_make_video_autoplay' ) ) {
	function chinchilla_make_video_autoplay( $video, $muted = false ) {
...
if ( ! function_exists( 'chinchilla_get_embed_video' ) ) {
	function chinchilla_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = chinchilla_make_video_autoplay( $embed_video );
		} else if ( chinchilla_is_from_uploads( $video ) ) {
			...
			$width  = chinchilla_get_content_width();
			$height = round( $width / $ratio[0] * $ratio[1] );
			$dim    = apply_filters( 'chinchilla_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									) );
...
if ( ! function_exists( 'chinchilla_get_embed_video_tag_name' ) ) {
	function chinchilla_get_embed_video_tag_name() {
...
if ( ! function_exists( 'chinchilla_is_from_uploads' ) ) {
	function chinchilla_is_from_uploads( $url ) {
		$url          = chinchilla_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
...
if ( ! function_exists( 'chinchilla_is_youtube_url' ) ) {
	function chinchilla_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

// Check if URL from Vimeo
if ( ! function_exists( 'chinchilla_is_vimeo_url' ) ) {
	function chinchilla_is_vimeo_url( $url ) {
...
------------------------------------------------------------------- */
if ( ! function_exists( 'chinchilla_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'chinchilla_init_filesystem', 0 );
	function chinchilla_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'chinchilla_fpc' ) ) {
	function chinchilla_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'chinchilla' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'chinchilla_fgc' ) ) {
	function chinchilla_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'chinchilla' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'chinchilla_fga' ) ) {
	function chinchilla_fga( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'chinchilla' ), $file ) );
					}
				}
			}
		}
...
// Create a directory
if ( ! function_exists( 'chinchilla_mkdir' ) ) {
	function chinchilla_mkdir( $path ) {
		if ( ! empty( $path ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'chinchilla' ), $path ) );
							}
						}
					}
				} else {
					if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'chinchilla' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'chinchilla_unlink' ) ) {
	function chinchilla_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( chinchilla_is_on( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'chinchilla' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'chinchilla_copy' ) ) {
	function chinchilla_copy( $src, $dst ) {
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				return trx_addons_copy( $src, $dst );
			} else if ( function_exists( 'copy_dir' ) ) {
				$src = chinchilla_prepare_path( $src );
				$dst = chinchilla_prepare_path( $dst );
				return copy_dir( $src, $dst );
			} else {
				chinchilla_unlink( $dst );
				if ( is_dir( $src ) ) {
					if ( ! is_dir( $dst ) ) {
						chinchilla_mkdir( $dst );
					}
					$files = scandir( $src, SCANDIR_SORT_NONE );
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							chinchilla_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'chinchilla_retrieve_json' ) ) {
	function chinchilla_retrieve_json( $url ) {
		return function_exists( 'trx_addons_retrieve_json' ) ? trx_addons_retrieve_json( $url ) : '';
	}
}

// Remove unsafe characters from file/folder path
if ( ! function_exists( 'chinchilla_esc' ) ) {
	function chinchilla_esc( $name ) {
...
if ( ! function_exists( 'chinchilla_prepare_path' ) ) {
	function chinchilla_prepare_path( $path ) {
...
if ( ! function_exists( 'chinchilla_check_min_file' ) ) {
	function chinchilla_check_min_file( $file, $dir ) {
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && chinchilla_is_off( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && chinchilla_is_off( chinchilla_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'chinchilla_get_file_dir' ) ) {
	function chinchilla_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! CHINCHILLA_ALLOW_SKINS && ! chinchilla_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'chinchilla_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( CHINCHILLA_CHILD_DIR != CHINCHILLA_THEME_DIR && file_exists( CHINCHILLA_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? CHINCHILLA_CHILD_URL : CHINCHILLA_CHILD_DIR ) . chinchilla_check_min_file( $file, CHINCHILLA_CHILD_DIR );
			} elseif ( file_exists( CHINCHILLA_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? CHINCHILLA_THEME_URL : CHINCHILLA_THEME_DIR ) . chinchilla_check_min_file( $file, CHINCHILLA_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'chinchilla_get_file_url' ) ) {
	function chinchilla_get_file_url( $file ) {
		return chinchilla_get_file_dir( $file, true );
	}
}

// Return file extension from full name/path
if ( ! function_exists( 'chinchilla_get_file_ext' ) ) {
	function chinchilla_get_file_ext( $file ) {
...
if ( ! function_exists( 'chinchilla_get_file_name' ) ) {
	function chinchilla_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
...
if ( ! function_exists( 'chinchilla_get_folder_dir' ) ) {
	function chinchilla_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'chinchilla_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( CHINCHILLA_CHILD_DIR != CHINCHILLA_THEME_DIR && is_dir( CHINCHILLA_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? CHINCHILLA_CHILD_URL : CHINCHILLA_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( CHINCHILLA_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? CHINCHILLA_THEME_URL : CHINCHILLA_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'chinchilla_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'chinchilla_get_folder_url' ) ) {
	function chinchilla_get_folder_url( $folder ) {
		return chinchilla_get_folder_dir( $folder, true );
	}
}


// Merge all separate styles and scripts to the single file to increase page upload speed
if ( ! function_exists( 'chinchilla_merge_js' ) ) {
	function chinchilla_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= chinchilla_fgc( chinchilla_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = chinchilla_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = chinchilla_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			chinchilla_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'chinchilla' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'chinchilla_filter_js_output', apply_filters( 'chinchilla_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}
...
if ( ! function_exists( 'chinchilla_merge_css' ) ) {
	function chinchilla_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'chinchilla_filter_responsive_sizes', chinchilla_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = chinchilla_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = chinchilla_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'chinchilla_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'chinchilla_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'chinchilla' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'chinchilla' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = chinchilla_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			chinchilla_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'chinchilla') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'chinchilla_filter_css_output', apply_filters( 'chinchilla_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}
...
// Merge styles to the CSS file
if ( ! function_exists( 'chinchilla_filter_merge_css' ) ) {
	add_filter( 'chinchilla_filter_merge_css', 'chinchilla_filter_merge_css' );
	function chinchilla_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

// Merge styles to the SASS file
if ( ! function_exists( 'chinchilla_merge_sass' ) ) {
	function chinchilla_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'chinchilla_filter_responsive_sizes', chinchilla_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'chinchilla_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			chinchilla_fpc( chinchilla_get_file_dir( $to ), apply_filters( 'chinchilla_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'chinchilla_get_domain_from_url' ) ) {
	function chinchilla_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'chinchilla_add_protocol' ) ) {
	function chinchilla_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? chinchilla_get_protocol() . ':' . $url : $url;
	}
}
...
if ( ! function_exists( 'chinchilla_remove_protocol_from_url' ) ) {
	function chinchilla_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

// Add parameters to URL
if ( ! function_exists( 'chinchilla_add_to_url' ) ) {
	function chinchilla_add_to_url( $url, $prm ) {
...
if ( ! function_exists( 'chinchilla_is_url' ) ) {
	function chinchilla_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

// Check if string is external URL
if ( ! function_exists( 'chinchilla_is_external_url' ) ) {
	function chinchilla_is_external_url( $url ) {
		return chinchilla_is_url( $url ) && strpos( $url, chinchilla_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}
